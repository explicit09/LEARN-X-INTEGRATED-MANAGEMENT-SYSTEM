...
class DatabaseInitializer {
    constructor() {
        ...
        //   macOS: ~/Library/Application Support/LIMS/lims.db
        //   Windows: %APPDATA%\LIMS\lims.db
        this.dbPath = path.join(userDataPath, 'lims.db');
        this.dataDir = userDataPath;

        // The original DB path (read-only location in the package)
        this.sourceDbPath = app.isPackaged
            ? path.join(process.resourcesPath, 'data', 'lims.db')
            : path.join(app.getAppPath(), 'data', 'lims.db');
    }
