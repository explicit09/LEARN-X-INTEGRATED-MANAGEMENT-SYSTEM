...
const supabaseClient = require('./features/common/services/supabaseClient');
const supabaseDatabase = require('./features/common/services/supabaseDatabase');
...
function setupProtocolHandling() {
    ...
    try {
        if (!app.isDefaultProtocolClient('pickleglass')) {
            const success = app.setAsDefaultProtocolClient('lims');
            if (success) {
                console.log('[Protocol] Successfully set as default protocol client for lims://');
            } else {
                console.warn('[Protocol] Failed to set as default protocol client - this may affect deep linking');
            }
        } else {
            console.log('[Protocol] Already registered as default protocol client for lims://');
        }
    } catch (error) {
    ...
    app.on('second-instance', (event, commandLine, workingDirectory) => {
        ...
        for (const arg of commandLine) {
            if (arg && typeof arg === 'string' && arg.startsWith('lims://')) {
...
async function handleCustomUrl(url) {
    try {
        ...
        if (!url || typeof url !== 'string' || !url.startsWith('lims://')) {
